...
public class SvgLineupRenderer {
    ...
    private static final int BOARD_X = 12;
    private static final int BOARD_Y = 12;
    ...
    private static final int CANVAS_W = 760;
    ...
    private void appendHeader(StringBuilder svg, Board board) {
        ...
        if (board.experience() != null) {
            svg.append(String.format(
                    "<text x=\"%d\" y=\"%d\" fill=\"#aaa\" font-size=\"13\">经验 %s</text>\n",
                    BOARD_X + 80, headerY + 4, board.experience()));
        }
        svg.append(String.format(
                "<text x=\"%d\" y=\"%d\" fill=\"#f1c40f\" font-size=\"13\">金币 %d</text>\n",
    ...
    private void appendChampionCell(StringBuilder svg, int x, int y, Champion champ) {
        ...
        // cost badge
        svg.append(String.format(
                "<text x=\"%d\" y=\"%d\" fill=\"#fff\" font-size=\"10\" " +
                "text-anchor=\"end\">%s</text>\n",
                x + CELL_W - 5, y + 14, costLabel(champ.cost())));

        // champion name
        svg.append(String.format(
                "<text x=\"%d\" y=\"%d\" fill=\"#fff\" font-size=\"13\" " +
        ...
        svg.append(String.format(
                "<text x=\"%d\" y=\"%d\" fill=\"url(#starGold)\" font-size=\"12\" " +
        ...
        if (champ.items() != null && !champ.items().isEmpty()) {
            ...
            for (int i = 0; i < Math.min(champ.items().size(), 3); i++) {
                ...
                svg.append(String.format(
                        "<rect x=\"%d\" y=\"%d\" width=\"18\" height=\"18\" rx=\"3\" " +
                        "fill=\"%s\" stroke=\"#fff\" stroke-width=\"0.5\"/>\n",
                        ix, y + 52, itemColor));
                svg.append(String.format(
                        "<text x=\"%d\" y=\"%d\" fill=\"#fff\" font-size=\"9\" " +
                        "text-anchor=\"middle\">%s</text>\n",
    ...
    private void appendBench(StringBuilder svg, Board board) {
        ...
        svg.append(String.format(
                "<text x=\"%d\" y=\"%d\" fill=\"#888\" font-size=\"12\">备战席</text>\n",
                BOARD_X, benchY));
        ...
        for (int i = 0; i < Math.min(board.bench().size(), 9); i++) {
            ...
            svg.append(String.format(
                    "<text x=\"%d\" y=\"%d\" fill=\"#fff\" font-size=\"11\" " +
                    "text-anchor=\"middle\">%s</text>\n",
                    bx + bw / 2, benchY + 36, c.name()));
            String stars = "★".repeat(Math.min(c.star(), 3));
            svg.append(String.format(
                    "<text x=\"%d\" y=\"%d\" fill=\"#ffd700\" font-size=\"10\" " +
                    "text-anchor=\"middle\">%s</text>\n",
    ...
    private void appendRightPanel(StringBuilder svg, Board board) {
        ...
        // --- synergies ---
        svg.append(String.format(
                "<text x=\"%d\" y=\"%d\" fill=\"#ffd700\" font-size=\"14\" font-weight=\"bold\">羁绊</text>\n",
                px, py));
        py += lineH;
        ...
        if (board.activeSynergies() != null) {
            for (ActiveSynergy s : board.activeSynergies()) {
                svg.append(String.format(
                        "<text x=\"%d\" y=\"%d\" fill=\"#e0e0e0\" font-size=\"12\">%s</text>\n",
                        px, py, s.name()));
                if (s.description() != null) {
                    svg.append(String.format(
                            "<text x=\"%d\" y=\"%d\" fill=\"#888\" font-size=\"10\">%s</text>\n",
        ...
        if (board.itemPool() != null) {
            svg.append(String.format(
                    "<text x=\"%d\" y=\"%d\" fill=\"#ffd700\" font-size=\"14\" font-weight=\"bold\">装备池</text>\n",
                    px, py));
            py += lineH;
            ...
            if (pool.components() != null && !pool.components().isEmpty()) {
                svg.append(String.format(
                        "<text x=\"%d\" y=\"%d\" fill=\"#aaa\" font-size=\"11\">散件:</text>\n",
                        px, py));
                svg.append(String.format(
                        "<text x=\"%d\" y=\"%d\" fill=\"#e0e0e0\" font-size=\"11\">%s</text>\n",
                        px + 40, py, String.join(" ", pool.components())));
                py += lineH;
            }
            if (pool.completed() != null && !pool.completed().isEmpty()) {
                svg.append(String.format(
                        "<text x=\"%d\" y=\"%d\" fill=\"#aaa\" font-size=\"11\">成装:</text>\n",
                        px, py));
                svg.append(String.format(
                        "<text x=\"%d\" y=\"%d\" fill=\"#ffd700\" font-size=\"11\">%s</text>\n",
